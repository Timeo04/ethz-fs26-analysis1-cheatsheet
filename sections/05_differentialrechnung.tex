...
\subsubsection{Einseitige Ableitungen}
...
{\small \color{gray}($\lim_{h \rightarrow 0}\frac{f(x+h)-f(x)}{h}$, h darf positiv oder negativ sein, rechts- und linksseitiger Grenzwert muss definiert sein und übereinstimmen)}
...
\begin{itemize}
    \item $\ln'(x)= \frac{1}{x}$, $\ln'(x+a) = \frac{1}{x+a}, x+a \geq 0, a \in \mathbb{R}, a \neq -x$

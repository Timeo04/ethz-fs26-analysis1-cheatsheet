...
Für grosse $n$: $n! \approx \sqrt{2 \pi n} \cdot (\frac{n}{e})^n$
...
\begin{tabular}{|c|c|c|c|}
	\hline
	& \textbf{in terms of $\sin\theta$} & \textbf{in terms of $\cos\theta$} & \textbf{in terms of $\tan\theta$} \\ \hline
	$\sin\theta =$ & $\sin\theta$ & $\pm\sqrt{1-\cos^2\theta}$ & $\pm\dfrac{\tan\theta}{\sqrt{1+\tan^2\theta}}$ \\ \hline
	$\cos\theta =$ & $\pm\sqrt{1-\sin^2\theta}$ & $\cos\theta$ & $\pm\dfrac{1}{\sqrt{1+\tan^2\theta}}$ \\ \hline
	$\tan\theta =$ & $\pm\dfrac{\sin\theta}{\sqrt{1-\sin^2\theta}}$ & $\pm\dfrac{\sqrt{1-\cos^2\theta}}{\cos\theta}$ & $\tan\theta$ \\ \hline
\end{tabular}
\end{center}
...
\begin{center}
        \begin{tabular}{c||c}
	        ...
	        $ \text{arctanh}(x) $ & $ \frac{1}{1 - x^2} $ \\
                ...
                $ f(x)^{g(x)} $ &  $ f(x)^{g(x)} \cdot (\ln(f(x)) \cdot g(x))' $ \\
        \end{tabular}
...
Jede rationale Funktion $R(s)=\frac{Z(s)}{N(s)}$, wobei $Z(s)$ und $N(s)\neq 0$ Polynome sind (reelle Koeffizienten) kann umgeschrieben werden als $R(s)=\frac{Z(s)}{(s-s_1)\dots(s-s_m)}=\frac{a_1}{s-s_1}+\dots+\frac{a_m}{s-s_m}$, wobei $s_1, \dots, s_m$ Polstellen von $R(s)$ sind.

Weben dem Fundamentalsatz der Algebra können wir $N(s)$ immer schreiben als $(s-s_1)\dots(s-s_m)$.

\theorem{Partialbruchzerlegung: Umsetzung} (alle Polstellen einfach reell)
Für jede Nulstelle $s_i$ setzen wir Summanden $\frac{a_i}{s-s_i}$ an, also $R(s)=\frac{a_1}{s-s_1}+\frac{a_2}{s-s_2}+ \dots + \frac{a_m}{s-s_m}$.
Koeffizienten $a_i$ berechnet mit Formel $a_i=(R(s)(s-s_i))|_{s=s_i}$ oder über Koeffizientenvergleich.